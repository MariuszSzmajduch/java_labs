package laboratory05Streamed.Submission5_1;
...
public class LotteryNumbers {
	 ...
	 */
	private final String typeOfDraw;
   
   /**
    * Returns new {@link LotteryNumber} object 
    * containing an array of six random distinct integer 
   ...
   public String status(LotteryNumbers draw) {
	   int match = (int)IntStream.of(draw.ln)
   ...
   private void generateTicket() {
	   this.ln = IntStream.generate(() -> (int)(Math.random()* 49 + 1))
			   			.distinct()
			   			.limit(6)
			   			.sorted()
			   			.toArray();
   ...
   private void convert (String ticket) {
	   this.ln = Stream.of(ticket.split(" "))
			   		.filter(s -> !s.isEmpty())
